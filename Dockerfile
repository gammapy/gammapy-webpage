...
COPY binder.py tmp/
RUN curl -o tmp/environment.yml https://gammapy.org/download/install/gammapy-0.14-environment.yml
...
RUN gammapy download notebooks --out=${HOME}/gammapy-tutorials --release=0.14
RUN gammapy download datasets --out=${HOME}/gammapy-datasets --release=0.14
...
USER ${NB_USER}
WORKDIR ${HOME}/gammapy-tutorials/notebooks-0.14
