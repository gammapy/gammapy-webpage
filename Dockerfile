...
COPY binder.py tmp/
RUN curl -o tmp/environment.yml https://gammapy.org/download/install/gammapy-0.13-environment.yml
...
RUN gammapy download notebooks --out=${HOME}/gammapy-tutorials --release=0.13
RUN gammapy download datasets --out=${HOME}/gammapy-datasets --release=0.13
...
USER ${NB_USER}
WORKDIR ${HOME}/gammapy-tutorials/notebooks-0.13
